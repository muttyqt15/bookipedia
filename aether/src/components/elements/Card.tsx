...
const Card = ({ id, title, description, image, author }: CardProps) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="text-[#3E3232] hover:scale-95 relative flex flex-col w-64 h-96 bg-[#C19A6B] border rounded-xl border-none text-2xl m-4 p-2 hover:shadow-md transition-all">
      ...
      <p className="text-xs absolute bottom-2 right-2">{author.username}</p>
    </button>
  );
};
